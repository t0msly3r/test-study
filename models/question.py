...
class Question:
    def __init__(self, id=None, subject_id=None, text=None,
                 option_a=None, option_b=None, option_c=None, correct_answer=None):
        self.id = id
    ...
    def get_by_subject(subject_id):
        ...
        cursor = conn.cursor()
        cursor.execute(
            """SELECT id, subject_id, text, option_a, option_b, option_c, correct_answer
    ...
    def add_many(questions_data):
        ...
        cursor = conn.cursor()
        cursor.executemany(
            """INSERT INTO questions (subject_id, text, option_a, option_b, option_c, correct_answer)
               VALUES (?, ?, ?, ?, ?, ?)""",
            questions_data
        )
        conn.commit()
